...
= Methodological Approach
  This thesis project started as an exploratory data analysis project, trying to find easy to implement metrics for public transit service coverage and accessibility with transit based on open source software and openly accessible data. Starting points were prior considerations about closeness centrality and reach, as well as an interest in the temporal variability of transit services on the macro and micro scale. 
  ...
  == Case Studies <case-study>
    ...
    First attempts at routing were based in Heidelberg, Germany arbitrarily. After fine tuning the process in Bonn, Germany, I made the decision to use Heidelberg again for the final data for this thesis, based on my personal familiarity with Heidelberg and its transit network. As this thesis does not include any measures to verify the data acquired through routing with an empircal sample of real life experiences, personal experience and familiarity at least allowed for checks against my own experience and intuition.

  == Data Acquisition <data>
    As mentioned in @case-study above, there were several requirements for data to process. Transportation data was required in two forms, both for the underlying street network, as well as for the public transit schedule data (see @transit). Beyond that routing needed origins and destinations (see @endpoints).
    ...
    === Transport Data <transit>
      ...
      - but ultimately settled on cropping the german weekly transit dataset from DELFI @delfi_deutschlandweite_2023 using gtfs-general.
